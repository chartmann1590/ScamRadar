...
import com.charles.scamradar.app.download.DownloadState
...
fun SettingsScreen(
    ...
    LaunchedEffect(downloadProgress.state) {
        if (downloadProgress.state == DownloadState.COMPLETED) {
            modelRefreshKey++
        }
    }
        ...
        Card(
            ...
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                if (isDownloading) {
                    ...
                    val downloaded = downloadProgress.bytesDownloaded
                    ...
                    val mbDone = (downloaded / (1024.0 * 1024.0)).toInt()
                    val mbTotal = if (total > 0L) (total / (1024.0 * 1024.0)).toInt() else 0
                    Text(
                        text = if (downloadProgress.state == DownloadState.PAUSED) {
                            "Paused • $mbDone / $mbTotal MB"
                        } else {
                            "${(fraction * 100).toInt()}% • $mbDone / $mbTotal MB"
                        },
